...
#include "tier0/platform.h"
	...
	PLATFORM_INTERFACE int g_Telemetry[70];

	PLATFORM_INTERFACE inline void TelemetryTick() { }
	PLATFORM_INTERFACE inline void TelemetrySetLevel( unsigned int ) { }
	...
	struct TelemetryData {
		...
		/// Conversion from tmFastTime() (rdtsc) to milliseconds.
		float flRDTSCToMilliSeconds;
		/// Count of frames to capture before turning off.
		uint32 FrameCount;
		/// Server name to connect to.
		char ServerAddress[128];
		/// GetPlaybackTick() value from demo file (or 0 if not playing a demo).
		int playbacktick;
		/// Start telemetry on demo tick #
		uint32 DemoTickStart;
		/// End telemetry on demo tick #
		uint32 DemoTickEnd;
		/// Current Telemetry level (Use TelemetrySetLevel to modify)
		uint32 Level;
	};
	...
	#define TelemetrySetLockName( _ctx, _location, _description ) \
		do {   													  \
			static bool s_bNameSet{}; 					          \
			if ( _ctx and not s_bNameSet ) {   					  \
	...
	public:
		CTelemetryLock( void* plocation, const char* description ) {
			m_plocation = static_cast<const char*>( plocation );
