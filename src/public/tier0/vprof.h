...
//-----------------------------------------------------------------------------
	...
	public:
		...
		void SetBudgetGroupID( const int id ) {
			m_BudgetGroupID = id;
		}
		...
		void SetClientData( const int iClientData ) { m_iClientData = iClientData; }
		[[nodiscard]]
	...
	private:
		void SetUniqueNodeID( const int id ) {
			m_iUniqueNodeID = id;
		}
	...
	public:
		...
		void SetTargetThreadId( const unsigned id ) { m_TargetThreadId = id; }
		...
		void HideBudgetGroup( const char* pszName, const bool bHide = true ) { HideBudgetGroup( BudgetGroupNameToBudgetGroupID( pszName), bHide ); }
	...
	inline CVProfScope::CVProfScope( const tchar*  pszName, const int detailLevel, const tchar* pBudgetGroupName, const bool bAssertAccounted, const int budgetFlags )
	...
	class CVProfCounter {
	public:
		explicit CVProfCounter( const tchar* pName, CounterGroup_t group = COUNTER_GROUP_DEFAULT ) {
