...
#include "tier0/dbg.h"
	...
	DBG_INTERFACE
	const char* BuildCmdLine( int argc, char* *argv, bool fAddSteam = true );
...
public:
	virtual auto ErrorMessage( const tchar* pMsg ) -> void = 0;
	virtual auto GetMainWindow() -> void*  = 0;
};
...
public:
	virtual auto ReadEvent() -> VCREvent = 0;
	virtual auto Read( void* pDest, int size ) -> void = 0;
};
...
struct VCR_t {
	...
	int			(*Hook_PeekMessage)( struct tagMSG* msg, void* hWnd, unsigned int wMsgFilterMin, unsigned int wMsgFilterMax, unsigned int wRemoveMsg );
	...
	void		(*Hook_ScreenToClient)( void* hWnd, tagPOINT* pt );
	...
	void*		(*Hook_CreateThread)( void* lpThreadAttributes, unsigned long dwStackSize, void* lpStartAddress, void* lpParameter, unsigned long dwCreationFlags, unsigned long* lpThreadID );
	...
	#define VCRHook_WaitForMultipleObjects( a, b, c, d ) g_pVCR->Hook_WaitForMultipleObjects( a, static_cast<const void**>( b ), c, d)
#else
