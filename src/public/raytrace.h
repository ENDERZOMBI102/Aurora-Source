...
public:
	...
	/// Returns direction sign mask for 4 rays. returns -1 if the rays can not be traced as a bundle.
	[[nodiscard]]
...
struct CacheOptimizedTriangle {
	...
	void ChangeIntoIntersectionFormat(); // change information storage format for
...
struct CacheOptimizedKDNode {
	...
	inline int32 TriangleIndexStart() const {
...
public:
	...
	void Trace4Rays( const FourRays& rays, fltx4 TMin, fltx4 TMax, RayTracingResult* rslt_out, int32 skip_id = -1, ITransparentTriangleCallback* pCallback = nullptr );
	...
	/// @param width width of desired rendering
	/// @param height height of desired rendering
	/// @param stride actual width in pixels of target buffer
	/// @param output_buffer pointer to destination
	/// @param CameraOrigin eye position
	/// @param ULCorner word space coordinates of upper left monitor corner
	/// @param URCorner top right corner
	/// @param LLCorner lower left
	/// @param LRCorner lower right
	void RenderScene( int width, int height, int stride, uint32* output_buffer, Vector CameraOrigin, Vector ULCorner, Vector URCorner, Vector LLCorner, Vector LRCorner, RayTraceLightingMode_t lightmode = DIRECT_LIGHTING );
	...
	float CalculateCostsOfSplit( int split_plane, int32 const* tri_list, int ntris, Vector MinBound, Vector MaxBound, float& split_value, int& nleft, int& nright, int& nboth );
	...
	/// @param position light center
	/// @param intensity rgb amount
	void AddInfinitePointLight( Vector position, Vector intensity );

	// use the global variables set by LoadBSPFile to populated the RayTracingEnvironment with faces.
	void InitializeFromLoadedBSP();
