	...
	for ( k = 0; k < MAXSTUDIOSKINS; k++ ) {
		if ( psource->mesh[ k ].numfaces ) {
			...
			for ( i = psource->mesh[ k ].faceoffset; i < psource->mesh[ k ].numfaces + psource->mesh[ k ].faceoffset; i++ ) {
				...
				Assert( ( ( psource->face[ i ].a & 0xF0000000 ) == 0 ) && ( ( psource->face[ i ].b & 0xF0000000 ) == 0 ) && ( ( psource->face[ i ].c & 0xF0000000 ) == 0 ) );
...
int checkCommand( char* str, const char* cmd, int numOptions, int numSplit ) {
	if ( strcmp( str, cmd ) == 0 ) {
