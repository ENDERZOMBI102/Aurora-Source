...
bool BuildRules( ISpRecoGrammar* cpRecoGrammar, SPSTATEHANDLE* root, CUtlVector< WORDRULETYPE >* rules ) {
	...
	if ( numrules > 1 ) {
		for ( int skip = 1; skip <= std::min( MAX_WORD_SKIP, numrules ); skip++ ) {
...
SR_RESULT ExtractPhonemes( const char* wavname, CSpDynamicString& text, CSentence& sentence, void ( *pfnPrint )( const char* fmt, ... ) ) {
	...
	const ULONGLONG ullInterest =
		SPFEI( SPEI_RECOGNITION ) | SPFEI( SPEI_END_SR_STREAM ) | SPFEI( SPEI_FALSE_RECOGNITION ) | SPFEI( SPEI_PHRASE_START ) | SPFEI( SPEI_HYPOTHESIS ) | SPFEI( SPEI_INTERFERENCE );
...
void MergeWords( CWordTag* w1, CWordTag* w2 ) {
	...
	start = std::min( w1->m_uiStartByte, w2->m_uiStartByte );
	end = std::max( w1->m_uiEndByte, w2->m_uiEndByte );
...
EXPOSE_SINGLE_INTERFACE_T( CPhonemeExtractorSAPI, IPhonemeExtractor )
