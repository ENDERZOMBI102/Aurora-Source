...
FORCEINLINE
GLuint GLMContext::FindSamplerObject( const GLMTexSamplingParams& desiredParams ) {
	int h = bitmix32( desiredParams.m_bits + desiredParams.m_borderColor ) & cSamplerObjectHashSize - 1;
	while ( m_samplerObjectHash[ h ].m_params.m_bits != desiredParams.m_bits or m_samplerObjectHash[ h ].m_params.m_borderColor != desiredParams.m_borderColor ) {
		if ( not m_samplerObjectHash[ h ].m_params.m_packed.m_isValid ) {
			break;
		}
...
FORCEINLINE
void GLMContext::FlushDrawStates( uint nStartIndex, uint nEndIndex, uint nBaseVertex ) { // shadersOn = true for draw calls, false for clear calls
	...
	GL_BATCH_PERF( m_FlushStats.m_nTotalBatchFlushes += 1; )
	...
	Assert( m_drawingProgram[kGLMVertexProgram]->m_type == kGLMVertexProgram and m_drawingProgram[kGLMFragmentProgram]->m_type == kGLMFragmentProgram );
	Assert( m_drawingProgram[ kGLMVertexProgram ]->m_bTranslatedProgram and m_drawingProgram[ kGLMFragmentProgram ]->m_bTranslatedProgram );
		...
		for ( int nSamplerIndex = 0; nSamplerIndex < GLM_SAMPLER_COUNT; ++nSamplerIndex, nCurMask <<= 1 ) {
			if ( !m_samplers[ nSamplerIndex ].m_pBoundTex ) continue;
			...
			if ( bShaderShadow ) {
				...
				if ( not bSamplerIsDepth or not bSamplerShadow ) {
	...
	if ( m_bDirtyPrograms ) {
		...
		if ( pNewPair != m_pBoundPair ) {
			...
			gGL->glUseProgram( static_cast<GLuint>( pNewPair->m_program ) );

			GL_BATCH_PERF( m_FlushStats.m_nTotalProgramPairChanges += 1; )

			if ( not m_pBoundPair ) {
				GL_BATCH_PERF( m_FlushStats.m_nNewPS += 1; )
				GL_BATCH_PERF( m_FlushStats.m_nNewVS += 1; )
			} else {
				GL_BATCH_PERF( if ( pNewPair->m_fragmentProg != m_pBoundPair->m_fragmentProg ) m_FlushStats.m_nNewPS += 1; )
				GL_BATCH_PERF( if ( pNewPair->m_vertexProg != m_pBoundPair->m_vertexProg ) m_FlushStats.m_nNewVS += 1; )
			}
	...
	Assert( m_ViewportBox.GetData().width == static_cast<int>( m_ViewportBox.GetData().widthheight & 0xFFFF ) );
	Assert( m_ViewportBox.GetData().height == static_cast<int>( m_ViewportBox.GetData().widthheight >> 16 ) );
	...
	if ( m_bUseSamplerObjects ) {
		while ( m_nNumDirtySamplers ) {
			...
			Assert( nSamplerIndex < GLM_SAMPLER_COUNT and not m_nDirtySamplerFlags[nSamplerIndex] );

			m_nDirtySamplerFlags[ nSamplerIndex ] = 1;

			gGL->glBindSampler( nSamplerIndex, FindSamplerObject( m_samplers[ nSamplerIndex ].m_samp ) );

			GL_BATCH_PERF( m_FlushStats.m_nNumSamplingParamsChanged += 1 );

			#if defined( OSX )
				CGLMTex* pTex = m_samplers[ nSamplerIndex ].m_pBoundTex;

				if ( pTex and not gGL->m_bHave_GL_EXT_texture_sRGB_decode ) {
		...
		while ( m_nNumDirtySamplers ) {
			...
			if ( pTex and pTex->m_SamplingParams != m_samplers[ nSamplerIndex ].m_samp ) {
	...
	if ( m_bUseBoneUniformBuffers ) {
		...
		if ( m_programParamsF[ kGLMVertexProgram ].m_dirtySlotHighWaterNonBone ) {
			...
			int dirtySlotHighWater = std::min( m_drawingProgram[kGLMVertexProgram]->m_descs[kGLMGLSL].m_highWater, m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone );

			GLint vconstLoc = m_pBoundPair->m_locVertexParams;
			if ( vconstLoc >= 0 and dirtySlotHighWater > firstDirtySlot ) {
				#if GL_BATCH_TELEMETRY_ZONES
		...
		if ( m_programParamsF[ kGLMVertexProgram ].m_dirtySlotHighWaterBone ) {
			...
			if ( vconstBoneLoc >= 0 ) {
				...
				if ( m_drawingProgram[ kGLMVertexProgram ]->m_descs[ kGLMGLSL ].m_VSHighWaterBone > 0 and m_nMaxUsedVertexProgramConstantsHint > DXABSTRACT_VS_FIRST_BONE_SLOT ) { shaderSlotsBone = std::min( m_drawingProgram[kGLMVertexProgram]->m_descs[kGLMGLSL].m_VSHighWaterBone, m_nMaxUsedVertexProgramConstantsHint - DXABSTRACT_VS_FIRST_BONE_SLOT ); }

				int dirtySlotHighWaterBone = std::min( shaderSlotsBone, m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterBone );
				if ( dirtySlotHighWaterBone ) {
					...
					GL_BATCH_PERF( m_nTotalVSUniformBoneCalls += 1; )
		...
		if ( m_programParamsF[ kGLMVertexProgram ].m_dirtySlotHighWaterNonBone ) {
			...
			int firstDirtySlot = m_programParamsF[ kGLMVertexProgram ].m_firstDirtySlotNonBone;
			int dirtySlotHighWater = std::min( nMaxUsedShaderSlots, m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone );
			...
			if ( vconstLoc >= 0 and dirtySlotHighWater > firstDirtySlot ) {
				...
				GL_BATCH_PERF( m_nTotalVSUniformCalls += 1; )
				GL_BATCH_PERF( m_nTotalVSUniformsSet += dirtySlotHighWater - firstDirtySlot; )
	...
	if ( m_pBoundPair->m_bHasBoolOrIntUniforms ) {
		if ( m_programParamsB[ kGLMVertexProgram ].m_dirtySlotCount ) { // optimize this later after the float param pushes are proven out
			const uint nLimit = std::min( static_cast<uint>( CGLMShaderPair::cMaxVertexShaderBoolUniforms ), m_programParamsB[kGLMVertexProgram].m_dirtySlotCount );
			for ( uint i = 0; i < nLimit; i += 1 ) {
		...
		if ( m_programParamsB[ kGLMFragmentProgram ].m_dirtySlotCount ) { // optimize this later after the float param pushes are proven out
			const uint nLimit = std::min( static_cast<uint>( CGLMShaderPair::cMaxFragmentShaderBoolUniforms ), m_programParamsB[kGLMFragmentProgram].m_dirtySlotCount );
			for ( uint i = 0; i < nLimit; i += 1 ) {
	...
	Assert( ( m_pDevice->m_streams[0].m_vtxBuffer and m_pDevice->m_streams[0].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[0] ) or ( not m_pDevice->m_streams[0].m_vtxBuffer and m_pDevice->m_vtx_buffers[0] == m_pDevice->m_pDummy_vtx_buffer ) );
	Assert( ( m_pDevice->m_streams[1].m_vtxBuffer and m_pDevice->m_streams[1].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[1] ) or ( not m_pDevice->m_streams[1].m_vtxBuffer and m_pDevice->m_vtx_buffers[1] == m_pDevice->m_pDummy_vtx_buffer ) );
	Assert( ( m_pDevice->m_streams[2].m_vtxBuffer and m_pDevice->m_streams[2].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[2] ) or ( not m_pDevice->m_streams[2].m_vtxBuffer and m_pDevice->m_vtx_buffers[2] == m_pDevice->m_pDummy_vtx_buffer ) );
	Assert( ( m_pDevice->m_streams[3].m_vtxBuffer and m_pDevice->m_streams[3].m_vtxBuffer->m_vtxBuffer == m_pDevice->m_vtx_buffers[3] ) or ( not m_pDevice->m_streams[3].m_vtxBuffer and m_pDevice->m_vtx_buffers[3] == m_pDevice->m_pDummy_vtx_buffer ) );

	uint nCurTotalBufferRevision{ m_pDevice->m_vtx_buffers[ 0 ]->m_nRevision + m_pDevice->m_vtx_buffers[ 1 ]->m_nRevision + m_pDevice->m_vtx_buffers[ 2 ]->m_nRevision + m_pDevice->m_vtx_buffers[ 3 ]->m_nRevision };
	...
	if ( nCurTotalBufferRevision != m_CurAttribs.m_nTotalBufferRevision or
	     m_CurAttribs.m_pVertDecl != m_pDevice->m_pVertDecl or
	     m_CurAttribs.m_vtxAttribMap[ 0 ] != reinterpret_cast<const uint64*>(m_pDevice->m_vertexShader->m_vtxAttribMap)[ 0 ] or
	     m_CurAttribs.m_vtxAttribMap[ 1 ] != reinterpret_cast<const uint64*>(m_pDevice->m_vertexShader->m_vtxAttribMap)[ 1 ] or
		...
		for ( int nMask = 1, nIndex = 0; nIndex < nMaxVertexAttributesToCheck; ++nIndex, nMask <<= 1 ) {
			...
			if ( pBuf == m_pDevice->m_pDummy_vtx_buffer ) {
				Assert( pStream->m_vtxBuffer == nullptr );
			...
			Assert( nBufOffset >= 0 );
			Assert( nBufOffset < static_cast<int>( pBuf->m_nSize ) );
			...
			if ( not ( m_lastKnownVertexAttribMask & nMask ) ) {
				m_lastKnownVertexAttribMask |= nMask;
				gGL->glEnableVertexAttribArray( nIndex );
			}
		}

		for ( int nIndex = nMaxVertexAttributesToCheck; nIndex < m_nNumSetVertexAttributes; nIndex += 1 ) {
			gGL->glDisableVertexAttribArray( nIndex );
	...
	if ( m_programParamsF[ kGLMFragmentProgram ].m_dirtySlotHighWaterNonBone ) {
		...
		if ( fconstLoc >= 0 ) {
			...
			int dirtySlotHighWater = std::min( nMaxUsedShaderSlots, m_programParamsF[kGLMFragmentProgram].m_dirtySlotHighWaterNonBone );
			...
			if ( dirtySlotHighWater > firstDirtySlot ) {
				...
				GL_BATCH_PERF( m_nTotalPSUniformCalls += 1; )
